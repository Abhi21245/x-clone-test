...
const getUserTweet = asyncHandler(async (req, res) => {
	...
	// const authorId = await User.findOne({ userName });

	// console.log("this is username", authroId);
	// return;
          ...
          '$match': {
            '$expr': {
              '$and': [
                {
                  '$eq': [
                    '$follower', new mongoose.Types.ObjectId(loggedInUser)
                  ]
                }, {
                  '$eq': [
                    '$following', new mongoose.Types.ObjectId(authorId)
                  ]
                }
              ]
            }
...
export { createTweet, deleteTweet, getfeedTweet, getUserTweet };
